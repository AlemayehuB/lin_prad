...
def mag_parse(fname, bin_um):
    ...
    while match('^#', line): line = fd.readline()
    ...
    nbins = int(ru.dmax * 2 / (bin_um/10000.0)) # number of bins per dimensions
...
def parse_im(fname):
    ...
    data = pd.read_csv(fname,header=None, comment='#',delimiter=",").as_matrix()
    ...
    return flux2D,flux2D_ref,s2r_cm,s2d_cm,Ep_MeV,bin_um
